...
python main.py --config configs/$CONFIG.yaml input=data/$TARGET_rgba.png save_path=$TARGET mesh_format=glb elevation=$ELEVATION force_cuda_rast=True
python main2.py --config configs/$CONFIG.yaml input=data/$TARGET_rgba.png save_path=$TARGET mesh_format=glb elevation=$ELEVATION force_cuda_rast=True
